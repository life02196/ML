...
# 使用 ViT-L/32 的預訓練權重
weights = ViT_L_32_Weights.IMAGENET21K_R

# 定義 CIFAR-100 數據集的轉換，根據 ViT-L/32 的預處理要求
...
# 加載預訓練的 ViT-L/32 模型並使用預訓練權重
model = vit_l_32(weights=weights)
model.heads.head = nn.Linear(model.heads.head.in_features, 10)  # 修改輸出層以適應 CIFAR-10 (10類)
