...
# 定義 CIFAR-100 數據集的轉換，根據 AlexNet 的預處理要求
...
# 加載預訓練的AlexNet模型並使用預訓練權重
model = alexnet(weights=weights)
model.classifier[6] = nn.Linear(model.classifier[6].in_features, 100)  # 修改輸出層以適應CIFAR-10 (10類)
...
def save_and_plot():
    ...
    plt.savefig(f'training_results_{current_time}.png')
    plt.show()

    torch.save(model.state_dict(), f'alexnet_cifar10_{current_time}.pth')

# 保存當前代碼到文件
def save_current_code():
    current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = f'current_code_{current_time}.py'
...
try:
    for epoch in range(num_epoch):
        ...
        if patience_counter >= 10:  # 如果測試損失在連續 20 次 epoch 中沒有改善，則停止訓練
